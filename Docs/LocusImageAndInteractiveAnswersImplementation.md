...
The tools are advertised only while a provider is configured, on both the classic and ChatGPT-native routes, and hidden in Plan mode on both routes; they appear only when the agent’s capability policy allows network and workspace write access, and a guessed call is refused at dispatch under the same rules. They are unavailable to Just Chat, private Identity tasks, role-contracted specialists, Solo helpers, and read-only agents. The “Render interactive answers” switch in the same Settings section follows the `interactive_answers_v1` capability alone; switching image generation off does not lock it.
