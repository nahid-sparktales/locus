...
| Sui data/execution | Alchemy GraphQL | QuickNode GraphQL; Foundation HTTPS endpoint in development only; full Base58 genesis checkpoint digest verified on each read; partial/error-bearing, oversized, cross-chain, stale, unstable-pagination, duplicate-asset, or misowned-object responses rejected; native and Coin totals reconcile coin objects and the address accumulator; unknown Coin types and owned non-Coin objects are quarantined; object discovery excludes BCS, display metadata, and media; signing/execution remains disabled; no new JSON-RPC dependency |
