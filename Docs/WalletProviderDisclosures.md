...
| Sui data/execution | Alchemy GraphQL | QuickNode GraphQL; Foundation HTTPS endpoint in development only; full Base58 genesis checkpoint digest verified on each read; partial/error-bearing, oversized, cross-chain, or stale responses rejected; native totals reconcile coin objects and the address accumulator; signing/execution remains disabled; no new JSON-RPC dependency |
