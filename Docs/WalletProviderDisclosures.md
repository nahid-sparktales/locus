...
| Solana RPC/indexing | Alchemy | QuickNode; genesis identity verified; native transfers use one reconstructed System Program instruction; classic SPL sends use one reconstructed `TransferChecked` and either verify the signer-derived recipient associated token account or create it with an exact idempotent Associated Token Program instruction after proving the address unallocated; SPL and Token-2022 balances use validated [`getTokenAccountsByOwner`](https://solana.com/docs/rpc/http/gettokenaccountsbyowner) raw integer amounts |
