...
| Sui data/execution | Alchemy GraphQL | QuickNode GraphQL; Foundation HTTPS endpoint in development only; full Base58 genesis checkpoint digest verified on each read; partial/error-bearing, oversized, cross-chain, stale, unstable-pagination, or duplicate-asset responses rejected; native and Coin totals reconcile coin objects and the address accumulator; unknown Coin types are quarantined; signing/execution remains disabled; no new JSON-RPC dependency |
