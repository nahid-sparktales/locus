...
The combined 2.6 native app and UI-test targets compile. The full native suite passed **1,344 tests with zero failures**; the backend suite passed **1,992 tests**. Mobile formatting, analysis, all **14 tests**, Android, and iOS simulator builds passed before its changes were merged.

Manual native inspection checked the sidebar, right Agent inspector, manager collection/detail, creation chooser, schedule form, and incoming-event form. Required fields enable creation only when complete, and cancellation protects edited drafts. That review led to smaller forms and explicit accessibility boundaries for nested forms. The synthetic fixture has no live automation backend, so refreshing it exposes an unavailable-server error rather than concealing failures.
