...
## Locus runtime adaptation

Use Locus's available file, shell, web, and agent tools for the equivalent upstream operations. Read supporting files relative to this skill directory. Graphify's Python CLI is an external dependency, not part of Locus's Python runtime. Check `graphify --help` when invoked; if missing, install into a workspace-local virtual environment from the pinned source `graphifyy @ git+https://github.com/Graphify-Labs/graphify@20a20d30d8e7eef77675651f0199d87f913bd3e7`. Use that environment's Python and CLI for the instructions below rather than modifying Locus's bundled interpreter or installing an unpinned version. Do not install hooks, start persistent watchers, or enable MCP integrations unless requested.
