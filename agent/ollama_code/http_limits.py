...
class RequestBodyLimitMiddleware:
    ...
    Buffering is bounded by the route's limit and happens before route invocation:
    ...
    async def __call__(self, scope: Scope, receive: Receive, send: Send) -> None:
        ...
        max_bytes = self.route_limits.get(
            (scope.get("method", ""), scope.get("path", "")), self.max_bytes
        )
        ...
        if length and (len(length) > len(str(max_bytes)) or int(length) > max_bytes):
