...
The agent advertises `stdio: true` and `sandboxed: false`: every transport is
available, including local command servers alongside HTTP/SSE. The `sandboxed`
key is still reported because the native client decodes it as a required
field, not because any build runs in a container.
