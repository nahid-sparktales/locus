---
name: apple-design
description: Apple's approach to interface design and fluid, physical motion, translated for native and web interfaces. Use when building or reviewing gesture-driven UI, spring animations, drag/swipe/sheet interactions, momentum and interruptible transitions, adaptive text and layout, settings or inspector lifecycle, translucent materials and depth, typography, reduced-motion, or the design foundations behind Apple-style interfaces.
---
...
- **Validate adaptive text in live geometry.** Build success does not prove intrinsic sizing. Exercise empty, single-line, wrapped, capped-or-scrolling, and narrow-container states in the running interface. Measure against the real available width or inspect the parent's size proposal instead of assuming an off-screen measurement view receives useful constraints.
- **Separate state ownership before wiring settings or inspectors.** Persistence, navigation, confirmation feedback, and dismissal are independent actions. Draft initialization may load values, but it must not invoke a save routine that also closes the presentation or emits user feedback. Click through every entry route once after initialization and again after editing.
...
### Pre-delivery interaction check

Before handing off an adaptive or stateful interface:

1. Run the real interface at representative widths with empty and realistic content.
2. Verify wrapped controls keep their footer, actions, and surrounding layout stable.
3. Open each settings or inspector route from a clean launch and confirm initialization does not save, toast, navigate, or dismiss.
4. Apply, discard, close, and reopen after staged edits; verify each transition has one owner and one visible result.
