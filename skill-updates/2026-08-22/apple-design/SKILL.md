...
- **Keep presentation separate from durable identity.** Titles and icons may evolve as a destination's purpose becomes clearer; preserve storage keys, shortcut identifiers, and automation anchors unless the underlying identity changes, and use an explicit migration when it does.
- **Persist declared intent, not just resulting activity.** When a person chooses a mode, category, or navigation scope before execution, store that choice as durable identity. Child activity and other execution output describe progress; they must not be the sole category discriminator.
...
- **Verify responsive work with production geometry.** Include at least one fixture that uses real column widths and one compact split-view case. Check native titlebar exclusion zones, primary-control clearance, and the continuity of any material surface behind window controls; relaxed test-only layouts are not sufficient evidence.
- **Resolve screenshot language before destructive edits.** Map phrases such as "the top three buttons" to the exact visible group using the supplied layout, and preserve adjacent plausible groups until the mapping is clear.
- **Run a pre-flight check before delivery.** Re-read the applicable rules in this skill, exercise production and compact layouts, and inspect the rendered interface—not only the source or relaxed fixtures—for clipping, control clearance, legibility, accessibility preferences, and continuity through interaction.
