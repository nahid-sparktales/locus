...
3. *Post-write verification:* after appending, count occurrences of the
   ...
   the race. Before resolving duplicate numbers, compare the two complete
   entry bodies: identical bodies indicate a same-writer retry and one copy
   may be removed; different bodies indicate a parallel-writer collision,
   so renumber your entry and delete neither record. The pattern for shared
   logs written by parallel agents is check-then-act-then-verify.
