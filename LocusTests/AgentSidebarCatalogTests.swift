...
final class AgentSidebarCatalogTests: XCTestCase {
    ...
    func testTaskPickerUsesEventReceivingChatOwnerInsteadOfLatestSideChat() {
        let owner = AgentProfile(name: "Jinbei", model: "fixture")
        let other = AgentProfile(name: "Luffy", model: "fixture")
        let definition = AgentDefinition.trigger(trigger())
        let receiving = taskChat("primary", kind: "event", profile: owner, primary: true)
        let side = taskChat("newer-side-chat", kind: "event", profile: other)
        XCTAssertEqual(AgentTaskPickerPresentation.ownerLabel(
            for: definition, definitions: [definition], sessions: [side, receiving], profiles: [other, owner]
        ), "Agent: Jinbei")
    }

    func testTaskPickerUsesSchedulePrimaryOwnerAndKeepsSameIDEventSeparate() {
        let owner = AgentProfile(name: "Garp", model: "fixture")
        let other = AgentProfile(name: "Law", model: "fixture")
        let definition = AgentDefinition.schedule(schedule())
        let receiving = taskChat("schedule-primary", kind: "schedule", profile: owner, primary: true)
        let side = taskChat("schedule-side-chat", kind: "schedule", profile: other)
        let event = taskChat("primary", kind: "event", profile: other, primary: true)
        XCTAssertEqual(AgentTaskPickerPresentation.ownerLabel(
            for: definition, definitions: [definition, .trigger(trigger())],
            sessions: [event, side, receiving], profiles: [other, owner]
        ), "Agent: Garp")
    }

    func testTaskPickerDistinguishesMissingAgentFromUnassignedOrUnknownOwner() {
        let owner = AgentProfile(name: "Removed", model: "fixture")
        let definition = AgentDefinition.trigger(trigger())
        XCTAssertEqual(AgentTaskPickerPresentation.ownerLabel(
            for: definition, definitions: [definition],
            sessions: [taskChat("primary", kind: "event", profile: owner)], profiles: []
        ), "Unavailable agent")
        XCTAssertEqual(AgentTaskPickerPresentation.ownerLabel(
            for: definition, definitions: [definition],
            sessions: [taskChat("primary", kind: "event"), taskChat("side", kind: "event", profile: owner)],
            profiles: [owner]
        ), "No agent linked", "A side chat must not provide ownership for an unassigned receiving chat")
        XCTAssertEqual(AgentTaskPickerPresentation.ownerLabel(
            for: definition, definitions: [definition],
            sessions: [taskChat("side", kind: "event", profile: owner)], profiles: [owner]
        ), "Agent link unavailable", "A missing receiving chat must not borrow a side chat's owner")
    }

    func testTaskPickerDoesNotGuessBetweenConflictingScheduleOwners() {
        let first = AgentProfile(name: "First", model: "fixture")
        let second = AgentProfile(name: "Second", model: "fixture")
        let definition = AgentDefinition.schedule(schedule())
        XCTAssertEqual(AgentTaskPickerPresentation.ownerLabel(
            for: definition, definitions: [definition], sessions: [
                taskChat("first", kind: "schedule", profile: first),
                taskChat("second", kind: "schedule", profile: second),
            ], profiles: [first, second]
        ), "Agent link unavailable")
        XCTAssertEqual(AgentTaskPickerPresentation.ownerLabel(
            for: definition, definitions: [definition], sessions: [
                taskChat("owned", kind: "schedule", profile: first),
                taskChat("unassigned", kind: "schedule"),
            ], profiles: [first]
        ), "Agent link unavailable", "An unassigned chat is conflicting ownership, not evidence for the other agent")
    }

    private func taskChat(
        _ id: String, kind: String, profile: AgentProfile? = nil, primary: Bool = false
    ) -> SessionSummary {
        SessionSummary(id: id, name: id, preview: "", mtime: 1, size: 0,
                       agentTriggerID: "inbox", agentProfileID: profile?.id.uuidString.lowercased(),
                       agentKind: kind, agentPrimary: primary)
    }
