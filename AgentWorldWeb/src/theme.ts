...
export const ASSET_TYPES = ['resident', 'station', 'beacon', 'habitat', 'crates'] as const;
export type AssetType = typeof ASSET_TYPES[number];
export type Placement = { x: number; z: number; rotation?: number };
export type PropPlacement = Placement & { asset: 'beacon' | 'habitat' | 'crates' };
export type Theme = {
  version: 1;
  id: string;
  name: string;
  ...
  layout: { radius: number; stations: Placement[]; props: PropPlacement[] };
};
export const DEFAULT_THEME: Theme = {
  version: 1, id: 'outpost', name: 'Orbital Outpost', description: 'Your agents, among the stars.',
  assets: {}, heights: { resident: 1.7, station: 1.25, beacon: 3.5, habitat: 4, crates: 1.2 }, rotations: {},
  palette: { ground: '#647978', accent: '#7ce8d0', sky: '#132b37' },
  layout: { radius: 14.1, stations: [], props: [
    { asset: 'habitat', x: -10.5, z: -11.8, rotation: 0.3 },
    { asset: 'habitat', x: 10.5, z: -11.8, rotation: -0.3 },
    { asset: 'beacon', x: 0, z: -13.8 },
    { asset: 'crates', x: -11.5, z: 8, rotation: -0.4 },
    { asset: 'crates', x: 11.5, z: 8, rotation: 0.6 },
  ] },
};
...
export function parseTheme(input: unknown): Theme {
  ...
  const result: Theme = { ...DEFAULT_THEME, assets: {}, heights: { ...DEFAULT_THEME.heights }, rotations: {}, palette: { ...DEFAULT_THEME.palette }, layout: { ...DEFAULT_THEME.layout, stations: [], props: [...DEFAULT_THEME.layout.props] } };
  result.id = value.id;
  ...
  if (layout && typeof layout === 'object') {
    ...
    if (Array.isArray(layout.props) && layout.props.length <= 32 && layout.props.every(p => placement(p) && ['beacon', 'habitat', 'crates'].includes((p as PropPlacement).asset))) result.layout.props = layout.props.map(p => ({ asset: p.asset, x: p.x, z: p.z, ...(p.rotation === undefined ? {} : { rotation: p.rotation }) }));
  }
  return result;
}
