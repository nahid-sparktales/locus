...
def signature(path: Path) -> dict:
    ...
    for line in result.stderr.decode().splitlines():
        ...
        if name in {"CDHash", "TeamIdentifier", "Identifier"}:
            fields[name] = value
    # Apple's CDHash is the 20-byte code identity, not a full SHA-256 digest.
    require(
        bool(re.fullmatch(r"[0-9a-f]{40}", fields.get("CDHash", ""))),
        "missing 20-byte CodeDirectory identity",
    )
    require(
        bool(re.fullmatch(r"[A-Z0-9]{10}", fields.get("TeamIdentifier", ""))),
        "export has no signing team",
    )
    return fields
...
def source_identity(app: Path) -> tuple[str, str]:
    ...
    require(
        bool(re.fullmatch(r"[0-9a-f]{40}", source)), "app has no clean source revision"
    )
    provenance = dict(
        line.split("=", 1)
        for line in (app / "Contents/Resources/BuildProvenance.txt")
        .read_text()
        .splitlines()
        if "=" in line
    )
    require(
        provenance.get("source_revision") == source,
        "build and app source identities differ",
    )
    for relative in SIGNERS:
        signer_info = plist(app / relative / "Contents/Info.plist")
        require(
            signer_info.get("LocusSourceRevision") == source,
            "signer source identity differs",
        )
...
def validate_profile(signer: Path, team: str) -> str:
    ...
    require(
        profile_path.is_file(), "nested signer lacks its exported provisioning profile"
    )
    profile = plistlib.loads(
        run("/usr/bin/security", "cms", "-D", "-i", str(profile_path))
    )
    expiry = profile.get("ExpirationDate")
    require(isinstance(expiry, dt.datetime), "provisioning profile lacks expiry")
    require(
        expiry.replace(tzinfo=dt.timezone.utc) > dt.datetime.now(dt.timezone.utc),
        "provisioning profile expired",
    )
    require(
        team in profile.get("TeamIdentifier", []), "profile team differs from the app"
    )
    require(
        profile.get("ProvisionsAllDevices") is True,
        "signer requires a Developer ID distribution profile",
    )
    entitlements = profile.get("Entitlements", {})
    require(
        entitlements.get("get-task-allow", False) is False
    ...
    require(
        expected in groups or f"{team}.*" in groups,
        "profile does not authorize signer Keychain access",
    )
    identifier = entitlements.get(
        "com.apple.application-identifier", entitlements.get("application-identifier")
    )
    require(
        identifier in {expected, f"{team}.*"},
        "profile does not authorize the signer application",
    )
...
def snapshot(app: Path) -> dict:
    ...
    for relative in SIGNERS:
        ...
        require(
            identity["TeamIdentifier"] == outer["TeamIdentifier"], "signer team differs"
        )
        identity["profileSHA256"] = validate_profile(
            app / relative, outer["TeamIdentifier"]
        )
        signers[relative] = identity
    require(
        len({item["CDHash"] for item in signers.values()}) == 1,
        "signer copies have different code identities",
    )
    files = {}
...
def verify(receipt: dict, app: Path) -> dict:
    require(
        ...
        and receipt.get("exportMethod") == "developer-id",
        "not a Developer ID export receipt",
    )
    ...
    for relative, expected in receipt.get("files", {}).items():
        require(
            current["files"].get(relative) == expected,
            "exported bundle content changed",
        )
...
def main() -> None:
    ...
    identity = sub.add_parser("activation-identity")
    identity.add_argument("app", type=Path)
    identity.add_argument("zip", type=Path)
    args = parser.parse_args()
    if args.operation == "unsigned-digest":
        print(unsigned_digest(args.executable))
        return
    if args.operation == "activation-identity":
        current = snapshot(args.app)
        result = {key: current[key] for key in ("sourceRevision", "bundleVersion")}
        result.update(
            outerAppCodeDirectoryHash=current["outerApp"]["CDHash"],
            signerCodeDirectoryHash=next(iter(current["signers"].values()))["CDHash"],
            archiveSHA256=digest(args.zip),
        )
    ...
    require(
        options.get("method") == "developer-id"
    ...
    def signing_metadata(relative: str) -> bool:
        return "/_CodeSignature/" in relative or relative.endswith(
    ...
    for relative, expected in receipt["files"].items():
        ...
        if macho:
            ...
            require(
                ...
                "export added an executable absent from the archive",
            )
            require(
                unsigned_digest(archived_file) == unsigned_digest(exported_file),
                "export changed executable content",
            )
            ...
            require(
                (archived / relative).is_file()
                and digest(archived / relative) == expected,
                "export changed an archived resource",
            )
    receipt.update(
        {
            "schemaVersion": 1,
            ...
            "exportOptionsSHA256": digest(args.options),
            "archiveInfoSHA256": digest(args.archive / "Info.plist"),
        }
    )
